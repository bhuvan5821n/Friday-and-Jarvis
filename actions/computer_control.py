...
def _safe_screenshot_path(requested: str | None) -> Path:
    """Generate a timestamped screenshot path to avoid overwriting."""
    screenshots_dir = Path.home() / "Desktop" / "Jarvis Screenshots"
    screenshots_dir.mkdir(parents=True, exist_ok=True)
    
    fallback = screenshots_dir / f"screenshot_{time.strftime('%Y%m%d_%H%M%S')}.png"
    if not requested:
        return fallback
    try:
        ...
        for root in _SAFE_SCREENSHOT_ROOTS:
            if p.is_relative_to(root.resolve()):
                ...
                if p.is_dir():
                    ...
                    return p / f"screenshot_{time.strftime('%Y%m%d_%H%M%S')}.png"
...
def _type(text: str, interval: float = 0.03) -> str:
    """Type text with Unicode support using clipboard paste."""
    _require_pyautogui()
    time.sleep(0.3)
    
    # For ASCII-only text, use typewrite for speed
    if text.isascii():
        ...
        if _PYPERCLIP:
            ...
            # Fallback: try typing each character
            for char in text:
                try:
...
def _hotkey(*keys) -> str:
    """Execute a hotkey combination with safety checks."""
    _require_pyautogui()
    
    # Normalize keys for comparison
    normalized = {k.lower().strip() for k in keys if isinstance(k, str)}
    
    # Check for blocked combinations
    if normalized in _BLOCKED_HOTKEYS:
        return f"Blocked: {'+'.join(keys)} is a system shortcut and cannot be automated"
...
def _screen_find(description: str) -> tuple[int, int] | None:
    try:
        from core.ai import OmniModel, _MAX_IMG_SIDE

        _require_pyautogui()
        w, h  = pyautogui.size()
        img   = pyautogui.screenshot()
        # OmniModel downscales large images — model answers in the image it
        # sees, so scale its coordinates back to real screen space here.
        scale = max(w, h) / _MAX_IMG_SIDE if max(w, h) > _MAX_IMG_SIDE else 1.0
        ...
        response = OmniModel(task="vision").generate_content([img, prompt])
        ...
        if match:
            return int(int(match.group(1)) * scale), int(int(match.group(2)) * scale)
...
def _validate_coordinates(x, y) -> tuple[int, int]:
    """Validate and convert coordinates to safe integers."""
    try:
...
# Dangerous hotkey combinations that should be blocked
_BLOCKED_HOTKEYS = {
