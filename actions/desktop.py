...
#: Removed in AOCA Phase 1: `_build_sandbox`, `_execute_generated_code` and
#: `_ask_gemini_for_desktop_action`. They turned a spoken sentence into
#: model-written Python and ran it through `exec()`. The sandbox was not one —
#: it injected `ctypes`, which reaches any Windows API and re-imports anything,
#: so the restricted `__builtins__` bought nothing. Deleting the path is the
#: fix; a smaller allowlist would only have been a slower escape.
#:
#: The named desktop actions below (wallpaper, organize, clean, list, stats)
#: are unchanged and still cover every task the generated path was used for.

def _unsupported_task(task: str) -> str:
    """Say what is possible instead of improvising code to do it."""
    if not task:
        return "Please describe what you want to do on the desktop."
    return ("I can't do that one on the desktop. What I can do: set the "
            "wallpaper, organise or clean the desktop, list it, or report its "
            "stats. Nothing was run.")
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            # Free-form desktop tasks used to be answered with generated Python.
            # Now they are answered with the truth about what this tool can do.
            return _unsupported_task(task or params.get("description", ""))

        else:
            if action:
                return _unsupported_task(action)
