...
def _user_agent() -> str:
    """Generate a realistic user agent string based on the current Chrome version."""
    import subprocess
    
    # Try to get Chrome version
    chrome_version = "131.0.0.0"  # Default fallback
    
    try:
        if _OS == "Windows":
            # Try to read Chrome version from registry or file
...
class _BrowserSession:
    ...
    def __init__(self, browser_name: str, profile_directory: str | None = None):
        self.browser_name      = browser_name
        self.profile_directory = profile_directory   # e.g. "Default", "Profile 2"
    ...
    async def _launch(self):
        ...
        if self.profile_directory:
            # reuse a running Chrome window if it exposes a debug port
            # ponytail: only works when Chrome is started with
            # --remote-debugging-port=9222; otherwise falls through to launch
            try:
                browser = await engine_obj.connect_over_cdp(
                    "http://127.0.0.1:9222", timeout=1_500)
                self._context = (browser.contexts[0] if browser.contexts
                                 else await browser.new_context())
                self._page = await self._context.new_page()
                print("[Browser]  Attached to running Chrome via CDP")
                return
            except Exception:
                pass
        ...
        if self.profile_directory:
            kwargs["args"].append(f"--profile-directory={self.profile_directory}")
        ...
        label = (
            ...
            + (f" @ {exe}" if exe else "")
            + (f" [{self.profile_directory}]" if self.profile_directory else "")
        )
        ...
        try:
            ...
            print(f"[Browser] Falling back to JARVIS profile...")

        # Fallback to JARVIS profile when real profile is locked
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not launch {self.browser_name}. "
                f"Close any running {self.browser_name} windows and try again.\n"
                f"Error: {e2}"
            ) from e2
...
class _SessionRegistry:
    ...
    def _get_or_create(self, browser_name: str,
                       profile_directory: str | None = None) -> _BrowserSession:
        key = browser_name + (f"#{profile_directory}" if profile_directory else "")
        with self._lock:
            if key not in self._sessions:
                sess = _BrowserSession(browser_name, profile_directory)
                sess.start()
    ...
    def get(self, browser_name: str | None = None,
        ...
        sess = self._get_or_create(browser_name, profile_directory)
...
def browser_control(
    ...
    """One event per browser action. `action` is an enum; URLs are not sent."""
    ...
    emit("browser.started", EventState.EXECUTING, tool="browser_control",
         action=action)
    try:
        result = _browser_control(parameters, response, player, session_memory)
    except Exception as exc:
        emit("browser.finished", EventState.FAILED, tool="browser_control",
             action=action, error_code=type(exc).__name__,
             duration_ms=int((_time.monotonic() - started_at) * 1000))
        raise
    emit("browser.finished", EventState.EXECUTED, tool="browser_control",
         action=action, execution_started=True,
         duration_ms=int((_time.monotonic() - started_at) * 1000))
    return result


def _browser_control(
    parameters:    dict = None,
    response=None,
    player=None,
    session_memory=None,
