...
def _ddg_search(query: str, max_results: int = 6, retries: int = 3) -> list[dict]:
    """Search DuckDuckGo with retry logic for rate limiting."""
    try:
    ...
    for attempt in range(retries):
        try:
            ...
            if "rate" in str(e).lower() or "429" in str(e):
                ...
                print(f"[WebSearch] DDG rate limited, waiting {wait_time}s...")
                time.sleep(wait_time)
            else:
                raise
    
    print(f"[WebSearch] DDG failed after {retries} attempts: {last_error}")
    return []
