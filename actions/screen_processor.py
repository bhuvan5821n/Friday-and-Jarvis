...
def _get_voice_name() -> str:
    return os.environ.get(VOICE_NAME_ENV, _load_config().get("voice_name", DEFAULT_VOICE_NAME))


def _get_live_model() -> str:
    return os.environ.get(MODEL_NAME_ENV, _load_config().get("live_model", DEFAULT_LIVE_MODEL))


def _build_speech_config() -> gtypes.SpeechConfig:
    return gtypes.SpeechConfig(
        ...
        while True:
            try:
                ...
                async with client.aio.live.connect(
                    model=_get_live_model(), config=config
                ) as session:
...
def screen_process(
    ...
    try:
        ...
        error_msg = f"Could not start vision session: {e}"
        print(f"[Vision] ❌ {error_msg}")
        if player:
            player.write_log(f"Vision Error: {error_msg}")
        return False
